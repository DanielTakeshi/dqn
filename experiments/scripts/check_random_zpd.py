"""Designed to check the random ZPD intervals.
"""
import matplotlib
matplotlib.use('Agg')
...
def _criteria(x):
    ...
    I did not support randomizing ZPD until August 23, 2019.
    EEDIT: actually let's test with more b/c I want best ahead, etc.
    """
    MONTH_CUTOFF = 8
    DAY_CUTOFF = 1
...
def teacher_summary(ax,  exp_path, teacher_title, xvals, yvals, w, params,
    ...
    Sept 2019 TODO: use this script for better figure?
    ...
    ax[1,0].set_title("Student Matching Instances ({} times)".format(len(learner_rew)),
        fontsize=titlesize)
    ax[1,0].set_xlabel("Student Training Steps (Millions)", fontsize=xsize)
    ...
    ax[2,0].set_xlabel("Student Training Steps (Millions)", fontsize=xsize)
    ...
    ax[2,1].set_xlabel("Student Training Steps (Millions)", fontsize=xsize)
...
def plot(exp_path, nrows=4, ncols=2, w=100):
    ...
    # As of Sept 2019 these are the only ones I'm testing:
    with open(join(exp_path,'params.txt'), 'r') as f:
        params = json.load(f)
    op = params['teacher']['num_snapshot_ahead']
    mm = params['teacher']['overlap']['match_method']
    mb = params['teacher']['blend']['start']
    tf = params['train']['train_freq_per_step']
    env = params['env']['name']
    if mm != 'fixed_steps' or mb != 0.5 or GAME not in env or tf != 2:
        print('  skipping: {}'.format(exp_path))
        print('  due to: mb ratio {}, num ahead {}, mm {}, tf {}'.format(mb,op,mm,tf))
        return

    # Now make the figure, etc.
    ...
    ax[0,0].set_xlabel("Student Training Steps (Millions)", fontsize=xsize)
    ax[0,0].set_ylabel("Avg Past {} Rewards".format(w), fontsize=ysize)
    ax[0,0].set_title('{} (w={})'.format(title, w), fontsize=titlesize)

    # Load teacher model, load path, then plot data. Be careful we are allowed
    # to do this 'substitution' to get the expert data file ...
    ...
    # Some other information I stored.
    ...
    ax[1,1].set_xlabel("Student Training Steps (M)", fontsize=xsize)
    ax[1,1].set_ylabel("Student Greedy Epsilon", fontsize=ysize)
    ...
    ax[1,1].plot(i_steps, t_b, marker='x', color='red',
            label="Teacher Blend, last {:.2f}".format(t_b[-1]))
    ...
    ax[r,0].set_title("Raw Loss, Every k MB (size {}), w={}".format(bs, ww),
                      fontsize=titlesize)
    ax[r,0].set_xlabel("Student Training Steps (M)", fontsize=xsize)
    ...
    ax[r,1].set_xlabel("Student Training Steps (M)", fontsize=xsize)
    ...
    # Finally, save!!
    plt.tight_layout()
    lastname = 'fig_similar_to_before_{}.png'.format(title)
    figname = join(figdir,lastname)
    plt.savefig(figname)
    print("\nJust saved:  {}".format(figname))

    # --------------------------------------------------------------------------
    # TODO: make another plot here? This one hopefully can plot matching
    # methods. Let's do this here because it's hard to do it after the teacher.
    # --------------------------------------------------------------------------
    # Just get this from the teacher method ....
    # HACKY SOLUTION! Don't scale this up!
    assert GAME == 'SpaceInvaders', GAME
    ms = 15
    lw = 5
    eps = 0.01
    x = t_summary_res['x']
    teacher_title = t_summary_res['teacher_title']
    ...
    matched_num = t_summary_res['matched_num']
    ...
    fig, ax = plt.subplots(nrows, ncols, squeeze=False, sharex=True, sharey=True,
                           figsize=(11*ncols,8*nrows))
    if op == -1:
        ax[0,0].set_title("SpaceInvaders, Best Ahead".format(), fontsize=45)
    elif op == -2:
        ax[0,0].set_title("SpaceInvaders, Random Ahead".format(), fontsize=45)
    else:
        ax[0,0].set_title("SpaceInvaders, {} Ahead".format(op), fontsize=45)

    ax[0,0].set_xlabel("Student Training Steps (M)", fontsize=43)
    ax[0,0].set_ylabel("Avg Past {} Rewards".format(w), fontsize=43)
    ax[0,0].plot(x, learner_rew, ms=ms, lw=lw, color='black',
        label='Learner Rew.: {:.1f} to {:.1f}'.format(learner_rew[0], learner_rew[-1]))
        #label='Learner Rew, {:.1f} to {:.1f}'.format(learner_rew[0], learner_rew[-1]))
    #ax[0,0].plot(x, matched_rew, ms=ms, lw=lw, color='blue',
    #    label='Match Sshot, {:.1f} to {:.1f}'.format(matched_rew[0], matched_rew[-1]))
    ax[0,0].plot(x, next_sn_rew, ms=ms, lw=lw, color='red',
        label='ZPD Snapshot: {:.1f} to {:.1f}'.format(next_sn_rew[0], next_sn_rew[-1]))
    #ax[0,1].set_title(match_title, fontsize=titlesize)
    #ax[0,1].set_xlabel("Student Training Steps (Millions)", fontsize=xsize)
    #ax[0,1].set_ylabel("RAW Teacher Sshot Index", fontsize=xsize) # NOT condensed!
    #ax[0,1].plot(x, matched_num, marker='x', ms=ms, color='blue',
    #    label='Match Sshot Idx, {} to {}'.format(matched_num[0], matched_num[-1]))
    #ax[0,1].plot(x, next_sn_num, marker='x', ms=ms, color='red',
    #    label='ZPD Sshot Idx, {} to {}'.format(next_sn_num[0], next_sn_num[-1]))

    ax[0,0].set_xlim([0-eps, 6+eps])

    for row in range(nrows):
        for col in range(ncols):
            leg = ax[row,col].legend(loc="best", ncol=1, prop={'size':32})
            for legobj in leg.legendHandles:
                legobj.set_linewidth(5.0)
    ...
    lastname = 'fig_random_zpd_matching_{}.png'.format(title)
    plt.tight_layout()
    figname = join(figdir,lastname)
    plt.savefig(figname)
    print("Just saved:  {}\n".format(figname))
...
if __name__ == "__main__":
    ...
    dirs = sorted(
            [join(EXP_PATH,x) for x in os.listdir(EXP_PATH) if _criteria(x)]
    )
